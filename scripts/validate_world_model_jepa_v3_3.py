#!/usr/bin/env python3
"""Validate the preregistered OS-JEPA v3.3 policy without opening its final catalog."""
...
PROTOCOL = RESEARCH / "world_model_jepa_v3_3_protocol.json"
...
NEW_FINAL_SCENARIOS = RESEARCH / "world_model_incident_v3_3_final_catalog.json"
...
DEFAULT_RESULT = RESEARCH / "world_model_jepa_v3_3_validation.json"
...
def install_final_guard() -> dict:
    ...
    def audit(event: str, arguments: tuple) -> None:
        ...
        if opened in protected:
            ...
            raise PermissionError("v3.3 final catalog access is forbidden during policy validation")
...
def main() -> int:
    ...
    if not protocol["registered_before_policy_validation"] or protocol["final_scenario_open_count"] != 0:
        raise AssertionError("v3.3 policy protocol is not cleanly preregistered")
    ...
    v3_metrics = v3["rules_v3_3_plus_jepa_candidate"]["metrics"]
    legacy_metrics = legacy["rules_v3_3_plus_jepa_candidate"]["metrics"]
    gates = {
    ...
    if deployed_after != deployed_before:
        raise AssertionError("deployed files changed during v3.3 validation")
    ...
    result = {
        ...
        "development": {
            ...
            "v3_final_is_opened_development_for_v3_3": True,
        },
        "conditions": {
            "v3_opened_development": {
                "rules_v3_3": compact(v3["rules_v3_3"]["metrics"]),
                "rules_v3_3_plus_jepa_candidate": compact(v3_metrics),
            },
            "legacy_regression": {
                "rules_v3_3": compact(legacy["rules_v3_3"]["metrics"]),
                "rules_v3_3_plus_jepa_candidate": compact(legacy_metrics),
            },
        },
        "gates": gates,
