...
def main() -> None:
    ...
    require(
        report["test_metrics_used_for_selection"] is False,
        "candidate selection excluded every test partition",
    )
    require(
        digest(args.artifact) == report["candidate_artifact_sha256"],
        "artifact matches the frozen selected candidate",
    )
    require(
        digest(args.artifact) == baselines["artifact_sha256"],
        "baseline comparison uses the same candidate bytes",
    )
    require(
        report["protocol_id"] == protocol["protocol_id"],
        "selection report matches the registered protocol",
    )

    selected = report["selected_candidate"]
    header = struct.unpack("<4sIIIIIIIIffI", args.artifact.read_bytes()[:48])
    require(
        header[:7] == (b"PJE1", 1, 16, 7, 3, selected["latent"], selected["hidden"]),
        "artifact header records the selected v3 schema and capacity",
    )
    require(
        header[7] == selected["training_transitions"],
        "artifact header records all fitting transitions",
    )
    ...
    incident_misses = prior_promotion.misses(
        incident_test, weights, incident_test_metadata
    )
    stress_misses = prior_promotion.misses(stress_test, weights)
    require(
        len(base_misses)
        == report["candidate_test"]["original_test"]["diagnostics"]["rules_plus_jepa"][
            "fn"
        ],
        "ordinary false-negative decomposition is complete",
    )
    require(
        len(incident_misses)
    ...
    require(
        len(stress_misses)
        == report["candidate_test"]["stress_test"]["diagnostics"]["rules_plus_jepa"][
            "fn"
        ],
        "stress false-negative decomposition is complete",
    )
    ...
    evaluation = {
        ...
        + len(stress_train_metadata)
        + len(stress_validation_metadata)
        + len(stress_test_metadata),
        ...
        "transition_split": {
            ...
            + len(incident_validation)
            + len(stress_validation),
    ...
    false_negatives = {
        ...
        "ordinary": {
            ...
            "clusters": dict(
                sorted(
                    Counter("+".join(item["hazards"]) for item in base_misses).items()
                )
            ),
            "misses": base_misses,
        },
        ...
        "stress": {
            ...
            "clusters": dict(
                sorted(
                    Counter("+".join(item["hazards"]) for item in stress_misses).items()
                )
            ),
    ...
    args.false_negatives.write_text(
        json.dumps(false_negatives, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
