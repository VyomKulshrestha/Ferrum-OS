...
const serialLog = path.join(repo, "target", "socket-isolation-serial.log");
...
try {
  ...
  await sendText("write /tmp/init_test 7");
  await sendKey("ret");
  await sleep(400);
  await sendText("ring3 init");
  await sendKey("ret");

  await waitForSerial("[socket-test] capability denial confirmed", 30, start);
  check("process without network authority cannot create or close sockets", true);
  await waitForSerial("[socket-test] cross-process fd isolation confirmed", 30, start);
  check("network-capable process cannot operate another process descriptor", true);
  await waitForSerial("[socket-test] parent close result=0", 30, start);
  check("owner retains descriptor authority after attack", true);
  await waitForSerial("[socket-test] all checks complete", 30, start);
  await waitForSerial("FerrumOS:~$", 45, start);
  await waitForSerial("sockets=1", 30, start);
  check("process reaper closes leaked owner descriptors", true);
