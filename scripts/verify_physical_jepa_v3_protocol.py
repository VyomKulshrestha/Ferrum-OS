...
def main() -> None:
    ...
    require(protocol["registered_ood"] == {"protocol": "v2", "rows": 4_096, "seed": 20_260_825, "invalid_observations_fail_closed": True, "invalid_rows_excluded_from_transition_error": True}, "new OOD fixture and invalid-state accounting are fixed")
    require(len(candidates) == 4 and {item["training_seed"] for item in candidates} == {17, 42, 91}, "four candidates cover three seeds")
    require(all(item["stress_train_episodes"] == 3_000 for item in candidates), "every candidate fits the stress curriculum")
    require(max(item["latent"] for item in candidates) == 128 and max(item["hidden"] for item in candidates) == 256, "expanded capacity remains registered")
    require(set(protocol["selection"]["excludes"]) == {"base_test", "incident_test", "stress_test", "registered_ood"}, "selection excludes all four test partitions")
    require(protocol["selection"]["test_open_count"] == 1, "v3 permits one frozen-candidate test open")
