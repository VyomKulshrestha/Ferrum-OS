...
The answer is domain-dependent for prediction and negative for operational marginal caution in the 512-case catalogs. Physical JEPA is decisively better than the matched MLP and GRU on the frozen physical catalog. FerrumOS instead favors the GRU at H=1 and H=3 and the JEPA at H=5. On new paired temporal catalogs, both selected models respond in the correct counterfactual direction, but neither produces a learned-only intervention at the frozen zero-false-positive threshold. A later externally designed, locally executed Safety-Gymnasium benchmark separates a strong privileged planner-only controller from an active union that passes the registered naive-baseline gates but trades two additional completions for 14 additional hazard-cost steps relative to the planner; neither paired difference is statistically separated from zero. The work therefore strengthens the modeling and runtime-attribution evidence without manufacturing a learned-safety or planner-superiority claim.
...
All eight families are delayed, coupled, masked, or exogenous. Their danger labels materialize after the current state-action pair, while the deterministic predicates evaluate that present pair; `rule_block` is therefore false in all 1,024 domain-case records by construction of this estimand. Rules-only recall here is not an estimate of general rule quality. At the zero-FP-calibrated threshold, the learned branch also fails to extend the authority boundary into this delayed-hazard regime, so the union is uninformative because neither branch fires. This is a construct-coverage negative, not a failed verifier, and it does not support deployment promotion.
...
The v14 final opens untouched seeds 6000–6127 exactly once and compares five frozen arms. The privileged planner completes 121/128 tasks (94.53%) and records 70 hazard-cost events, an 87.08% reduction from the naive controller's 542 events. The selected active union completes 123/128 tasks (96.09%), changes 1.86% of commands, reaches 95.62% warning recall and 3.24% warning FPR on the 20-step nominal-controller oracle, and records 84 hazard-cost events, 84.50% below naive. Its effective-action recall is 55.00% (88/160). Executed intervention precision is 23.04% (88/382): 294 of 382 changed commands occur on oracle-labelled non-dangerous trajectories.

The canonical public summary is: **A privileged deterministic planner achieved 94.53% completion and 87.08% lower hazard cost than the naive controller; the active union passed every registered naive-baseline gate with 96.09% completion and 84.50% lower hazard cost, but incurred 14 more hazard-cost steps than the planner while completing two additional tasks.** The gates do not require superiority over the planner. Paired 10,000-resample episode-bootstrap intervals include zero for both the +1.5625 percentage-point completion difference (95% CI [-1.5625, 4.6875]) and the +14-step hazard-cost difference (95% CI [-22.025, 54.000]).

Every counted v14 intervention changes the applied action; the verifier recomputes all five arms, every raw union row, the exact final seeds, protocol and selection hashes, authority counters, and artifact digests. The 23.04% intervention precision is mechanistically consistent with the observed planner-relative cost increase, but the design does not identify causality and the paired interval includes zero. The retained v12 final fails its registered recall and realized-cost gates, and the later v13 Simplex amendment fails development without opening its reserved final seed range. Neither failure is overwritten by v14.
...
- an externally designed Safety-Gymnasium controller/shield benchmark with a strong planner-only arm, a separately recomputed active-union naive-baseline pass and planner-relative tradeoff, plus retained v12 and no-final-access v13 failures.
