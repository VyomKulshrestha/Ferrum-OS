...
The output-ablation v1 protocol fixed SafetyPointGoal1-v0, the full v14 risk adapter, threshold 0.72, tangent correction, final seeds 9000-9127, and two paired arms. The observed arm passes the three frozen Physical JEPA v5 outputs into the unchanged adapter. The masked arm computes the same outputs but replaces raw adapter features 22-24 with their preregistered development means immediately before scoring. A 10,000-resample paired episode bootstrap estimates observed-minus-masked changes. This is a direct closed-loop intervention on output values within one fixed software pipeline; it is not an architecture replacement or an architecture-wide causal claim.

Both 128-episode catalogs completed before a post-run protected-file check detected that another task had rebuilt the v1.1 PDF. The registered prospective v1 attempt therefore remains failed and result-ineligible. Recovery protocol v2 reused the retained catalogs without simulator execution and wrote the complete retrospective analysis. Verification amendment v3 corrects checkout-dependent newline hashing, validates the historical v1.1 PDF blob at the recovery registration commit, and passes 51/51 checks. It changes no estimand, metric, result, seed, model, adapter, or authority state and does not reclassify v1 as successful.
...
| Observed minus development-mean masked | Estimate | Paired bootstrap 95% CI | Interval conclusion |
|---|---:|---:|---|
...
Observed JEPA output values therefore cause more warnings and effective action changes in this fixed closed-loop software pipeline, but the statistically resolved rate changes are not evidence of benefit. The intervention increase accompanies lower precision, while completion, effective recall, and realized hazard-cost contrasts remain unresolved. The -10 observed hazard-step estimate is especially not a superiority result because its interval is wide and includes zero.

This table is a retrospective analysis of complete catalogs produced during a prospective execution that failed its final protected-file check. The failed v1 record remains result-ineligible; recovery v2 did not rerun the simulator; verification v3 recomputes the catalogs, scores, aggregates, paired effects, seeds, and feature intervention and passes all 51 checks. The evidence supports only the direct effect of observed-versus-masked output values in this fixed local pipeline. It does not provide prospective confirmation, architecture-wide causality, independent assessment, HIL, physical safety, deployment safety, or promotion eligibility.
...
The new 512-case catalogs show no interventions from either rules or learning, while the 3D stress intervenes on every case. The prospective Safety-Gymnasium result separates a useful privileged planner-only controller from an active union that passes the naive-baseline objective but trades higher completion for higher hazard cost relative to the planner. The retained-catalog output intervention then shows that observed JEPA values materially increase warning FPR and intervention rate while reducing intervention precision relative to mean masking, without resolving completion or realized hazard cost. These contrasts show why controller quality cannot be credited to a shield and why warning recall or intervention count cannot substitute for marginal executed outcomes. Deterministic authority must be evaluated with task completion, intervention, proposal recall, false positives, precision, realized cost, and controller divergence rather than a collision count alone.
...
13. The output-value ablation's prospective v1 execution failed after both catalogs completed because a protected report PDF changed concurrently. Recovery v2 is a subsequently registered retrospective analysis of those retained catalogs, not a successful prospective rerun. The paired estimates are conditional on the fixed seed set, planner, correction, threshold, and local simulator pipeline; the intervention does not isolate architecture or establish benefit.
...
The targeted local attribution, its registered post-hoc sensitivity checks, and the retrospective retained-catalog output-value analysis are complete. Because the output-ablation prospective v1 execution failed, its estimates are not prospective confirmation; repeating that claim would require a newly registered untouched seed range rather than relabelling the recovery. The higher-value evidence-class changes remain independent execution of a frozen controller/shield benchmark, actuator-disabled live HIL with physical clocks and interfaces, and externally controlled labels or assessment. For FerrumOS, independently operated longitudinal use and concurrent preview remain higher value than more synthetic prompts.

#### 12.3 Release rule

Technical Report v1.2 is the immutable evidence-changing successor to v1.1. Version 1.1 and its repository tag remain intact; v1.2 adds only the retained-catalog output-value evidence and the associated provenance boundary. The v1.2 freeze manifest binds the exact source, PDF, verifier, and evidence records; a repository tag may identify that freeze only after the committed objects exist. Future evidence-changing work must create a new immutable report version and retain failed frozen results. Promotion requires a separate prospective deployment protocol naming exact targets, rollback, capabilities, post-execution verification, and gates. Publication of this report is not that protocol.
...
| Observed JEPA output values are intervened on | Complete retained paired catalogs, seeds 9000-9127 | Observed values add 0.96 intervention points and reduce precision by 15.26 points; completion and hazard intervals include zero | Retrospective recovery, not prospective confirmation, architecture-wide causality, or benefit |
...
| Output-ablation v1 execution | Two feature interventions, seeds 9000-9127, 10,000 paired resamples | Both catalogs complete; final protected-report digest check fails | Prospective result ineligible; failure retained |
| Output-ablation recovery v2 / verification v3 | Retained catalog digests and unchanged estimands | 51/51 checks pass; aggregates, scores, features, seeds, and paired effects recompute | Retrospective analysis only; no simulator rerun, prospective reclassification, or promotion |
...
| Output ablation v1 | `6f70724`, 8 Sep | Observed frozen JEPA values versus preregistered development-mean masking; seeds 9000-9127 | Both catalogs completed; protected-report digest check failed; prospective result ineligible |
